...
library MapPartyBalance {

	struct Data {
		mapping(address => Model.Balance) map;
	...
	function add(Data storage self, address _key, Model.Balance memory _value) public {
	...
	function get(Data storage self, address _key) public view returns (Model.Balance storage) {
		return self.map[_key];
	}
	
	function getEntry(Data storage self, uint _index) public view returns (Model.Balance storage) {
